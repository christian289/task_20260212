...
### 4.4 JsonEmployeeParser "phone" 키 미지원 (심각도: 저)

`JsonEmployeeParser.KnownKeys`가 `["name", "email", "tel", "joined"]`로 정의되어 있어, JSON에서 `"phone"` 키를 사용하면 ExtraFields로 처리됨. `"tel"`과 `"phone"` 모두 지원해야 과제 예제 JSON과 일반적인 사용 모두 대응 가능.
...
| 3 | JsonEmployeeParser phone 키 추가 | 데이터 호환성 |
